...
test.describe('Quick add inline selectors', () => {
	test.describe('Todoist mode', () => {
		test('completes a @label token via Enter without submitting, then submits it with the label attached', async ({
			...
			await page.screenshot({path: testInfo.outputPath('01-todoist-label-popup.png'), fullPage: true})
			...
			await page.screenshot({path: testInfo.outputPath('02-todoist-label-selected.png'), fullPage: true})
		...
		test('lets a second Enter submit after selecting a mid-line token with trailing text', async ({
			authenticatedPage: page, apiContext, userToken,
		}) => {
			await updateUserSettings(apiContext, userToken, {
				frontendSettings: {quickAddMagicMode: 'todoist'},
			})
			await LabelFactory.create(1, {id: 1, title: 'groceries'})
			const project = (await ProjectFactory.create(1, {id: 1, title: 'Project A'}))[0]
			const views = await createDefaultViews(project.id)
			...
			const textarea = page.locator('.add-task-textarea')
			await textarea.fill('Buy @gro milk')

			// Move the caret inside the token, right after "gro" — the exact
			// regression case: select() used to leave the caret at the token's
			// end with text following it, so the keyup from the very Enter that
			// selected re-opened the popup and every later Enter kept re-selecting
			...
			const listbox = page.getByRole('listbox')
			await expect(listbox).toBeVisible()
			await expect(listbox.getByRole('option', {name: /groceries/})).toBeVisible()

			await textarea.press('Enter')
			await expect(textarea).toHaveValue('Buy @groceries milk')
			await expect(listbox).not.toBeVisible()
		...
		test('completes a #project token via Enter, quoting a multi-word project title', async ({
			...
			await page.screenshot({path: testInfo.outputPath('03-todoist-project-popup.png'), fullPage: true})
	...
	test.describe('Vikunja mode', () => {
		test('completes a *label token via Enter', async ({authenticatedPage: page, apiContext, userToken}, testInfo) => {
			await updateUserSettings(apiContext, userToken, {
			...
			await page.screenshot({path: testInfo.outputPath('04-vikunja-label-popup.png'), fullPage: true})
		...
		test('opens no popup for @, because it is the assignee prefix there, not labels', async ({
			...
			await page.screenshot({path: testInfo.outputPath('05-vikunja-at-does-nothing.png'), fullPage: true})
		})
	})
})
